...
function getLineChart(idx)
{
	...
	for(var i=0;i<chartInfo.data.datasets.length;i++){
	chartInfo.data.datasets[i].fill=false;
	
	chartInfo.data.datasets[i].lineTension=0;
	}
var chart=new Chart(ctx, 
chartInfo);
...
// function to display data in tables,F
...
function getValues1Multi(dt,idx)

{
	...
	var values=[];
	  for(var i=0; i< dt.values.length;i++)
		  {
		//  if(dts[i].name!="variable")
			  values.push(dt.values[i].value[idx]);
...
function getVariableMulti(dt,idx)
{	 
	
	  return dt.variable.split(",")[idx];
}
...
function getColors(num,colorNum){
	...
	for(var i=0; i< num;i++)
		{
		colors1.push("rgba("+colors[i]+","+colorNum+")");
...
function get2Colors(idx,num,colorNum){
	var colors1=[];
	for(var i=0; i< num;i++)
		{
		colors1.push("rgba("+colors2[idx]+","+colorNum+")");
		//Math.floor(Math.random() * 61) 
		}
	
	return colors1;
}
function getNoOfVariables(idx){
	
	return data[idx].variable.split(",").length;
}
...
var cdata={};	
  ...
  //  cdata.options.title.text= data[idx].variable;
   ...
   cdata.datasets= [];
   var numVar=getNoOfVariables(idx);
   
   for(var i=0;i<numVar;i++)
	   
{
	   var colorsTmp=numVar>1?get2Colors(i,cdata.labels.length,colorNum):getColors(cdata.labels.length,colorNum);
	   var borderColorsTemp=colorsTmp;
  cdata.datasets.push({
        label: numVar>1?getVariableMulti(data[idx],i):getVariable(data[idx],i),//data[idx].variable,
	   //label: "demo",
        data: numVar>1?getValues1Multi(data[idx],i):getValues1(data[idx]),
        		
        backgroundColor: colorsTmp
        /*[
...
function getPhysicianFilter(idx,curVal){
	
	   var str="<select class='btn btn-dasboard dropdown-toggle' data-toggle='dropdown' id='physician_"+idx+"' name='physician_"+idx+"'  style='width:300px;color:black'>"
	   renderedFilters.push("physician_"+idx);
	   str+=	"<option >---Choose physician-";//+surveyors[i].value;
	   
	  
	   for(var i=0;i<physician.length;i++){
			if(curVal==physician[i].name)
		str+=	"<option value='"+physician[i].name+"' selected>"+physician[i].value;
			else
				str+=	"<option value='"+physician[i].name+"'>"+physician[i].value;
		}
		str+="</select>";
		return str;
	}
		...
		if(filter[i]=="surveyors")
			...
			if(filter[i]=="areas"){
				...
				if(filter[i]=="ageType"){
					...
					if(filter[i]=="physicians"){
					
						str+="<br><label style='font-size:15px; color:black'> Physicians: </label>  "+getPhysicianFilter(idx,filterValues[idx].physician)+"";
					}
...
function getTableReport(idx){
	...
	for(var i=0; i<config[idx].rowHeadings.length;i++)
		...
		str+="<tr><td>"+(i+1)+"</td><td style='word-break:break-all'>"+config[idx].rowHeadings[i]+"</td></tr>";
		}
	}
...
for(var i=0; i<data[idx].values.length;i++)
	...
	if(i>=pageSize)
		style="display:none"
			...
			str+="<tr  id='"+rowId+"' style='"+style+"'><td>"+(i+1)+"</td>"
